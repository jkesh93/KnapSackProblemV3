...
namespace NapSackObject
{
    ...
    {
        // Global Variables;
        System.IO.StreamWriter sr = new System.IO.StreamWriter("output.txt");
        ...
        public void knapSack(int index, int profit, int weight)
        {
            ...
            Console.WriteLine("\nStep: " + counter + ". Visiting Node: (" + (index+1) + ", " + (loc[index+1]+1) + ")");
            sr.WriteLine("\nStep: " + counter + ". Visiting Node: (" + (index + 1) + ", " + (loc[index + 1] + 1) + ")");
            loc[index + 1] += 1;
            Console.WriteLine("Profit: " + profit + "\tWeight: " + weight);
            sr.WriteLine("Profit: " + profit + "\tWeight: " + weight);
            ...
            if (promising(index))
            {
                ...
                tryin[index + 1] = false;
                counter++;
                Console.WriteLine("\n Step: " + counter + ". BACKTRACKING TO NODE: (" + (index + 1) + ", " + (loc[index + 1]) + ")");
                sr.WriteLine("\n Step: " + counter + ". BACKTRACKING TO NODE: (" + (index + 1) + ", " + (loc[index + 1]) + ")");
        ...
        // knapSack helper function Promising;
            ...
            public bool promising(int index)
         {
            ...
            if(weight >= W)
            {
                ...
                promisingArray[index] = false;
                Console.WriteLine("Non-promising -- weight >= W");
                sr.WriteLine("Non-promising -- weight >= W");
                return false;
            }
            else
            {
               ...
               if(bound > maxProfit && counter != 1)
                {

                    promisingArray[index] = true;
                    Console.WriteLine("Promising");
                    sr.WriteLine("Promising");
                }
                if( !(bound > maxProfit) && counter != 1)
                {
                    promisingArray[index] = false;
                    Console.WriteLine("Non-promising -- bound < maxProfit");
                    sr.WriteLine("Non-promising -- bound < maxProfit");
                }
        ...
        public void viewNeat()
        {
            ...
            for (int i = 0; i < (data.Length - 1); i++)
            {
                ...
                sr.WriteLine("\t " + profits[i] + " \t   " + weights[i] + "\t\t\t" + priceWeight[i] + "\n");
            }
        }
        ...
        public void writeSolution()
        {
            ...
            for (int i = 0; i < resultArray.Length; i++)
            {
                ...
                sr.WriteLine("Include item " + (i + 1) + "?" + " " + resultArray[i]);
                if (resultArray[i])
                {
                ...
                sr.WriteLine("Profit: " + localProfit + "\t Weight: " + localWeight);
            }
            sr.Close();
            Console.ReadKey();
        }
